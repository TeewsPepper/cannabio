...
// ---------------- SESIONES ----------------
if (!process.env.SESSION_SECRET) throw new Error("SESSION_SECRET no definido");
...
// ---------------- CONFIGURACIÓN DE NODemailer ----------------
const DESTINATARIOS_PROD = ["dvidal@cannabiouy.com", "fsilveira@cannabiouy.com"];
...
app.get(
  ...
  (req: Request, res: Response) => {
    res.redirect(`${FRONTEND_URL}/transmision`);
  }
);
...
// ---------------- SESIONES / LOGOUT ----------------
app.get("/api/session", (req: Request, res: Response) => {
  if (req.isAuthenticated() && req.user) res.status(200).json({ authenticated: true, user: req.user });
  else res.status(401).json({ authenticated: false, user: null });
});
